...
<head>
    ...
    <title>ThaiNovel - อ่านนิยายออนไลน์</title>
...
<body class="bg-gray-50 text-gray-900 flex flex-col min-h-screen">
    ...
    <header class="bg-white shadow-sm sticky top-0 z-50">
        <div class="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
            ...
            <div class="flex-1 max-w-xl mx-4 hidden md:block">
                <div class="relative">
                    <input type="text" placeholder="ค้นหานิยาย, นักเขียน..." class="w-full bg-gray-100 rounded-full py-2 pl-10 pr-4 focus:outline-none focus:ring-2 focus:ring-green-500 transition-all">
                    <i class="fa-solid fa-magnifying-glass absolute left-4 top-3 text-gray-400"></i>
                </div>
            </div>

            <div class="flex items-center gap-3">
                <button class="text-gray-500 hover:text-green-600 font-medium px-4">เข้าสู่ระบบ</button>
            </div>
        </div>
    </header>
    ...
    <main id="app-content" class="flex-1 w-full max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
        <div class="py-20 text-center text-gray-500" id="loading-state">
            <i class="fa-solid fa-spinner fa-spin text-3xl mb-4"></i><br>กำลังดึงข้อมูลจาก Google Sheets...
        </div>
    </main>

    <footer class="bg-white border-t border-gray-200 mt-auto py-6">
        <div class="max-w-7xl mx-auto px-4 text-center text-gray-500 text-sm">
            &copy; 2026 ThaiNovel แพลตฟอร์มอ่านนิยายออนไลน์
        </div>
    </footer>

    <script>
        // 🔥 นำ URL ของ Web App จาก Google Apps Script มาใส่ที่นี่ 🔥
        ...
        async function loadDataFromExcel() {
            try {
                // ถ้ายังไม่ได้ใส่ URL ให้โชว์ข้อมูลจำลองไปก่อนเพื่อไม่ให้จอขาว
                if (GOOGLE_SHEET_API_URL === "ใส่_URL_ที่ได้จาก_Google_Apps_Script_ตรงนี้") {
                    document.getElementById('loading-state').innerHTML = '<i class="fa-solid fa-triangle-exclamation text-yellow-500 text-3xl mb-4"></i><br>กรุณาใส่ URL ของ Google Sheets ในโค้ดก่อนครับ';
                    return;
                }
                ...
                // แปลงหมวดหมู่จาก String ("แฟนตาซี, ระบบ") ให้เป็น Array
                ...
                document.getElementById('loading-state').innerHTML = '<i class="fa-solid fa-circle-xmark text-red-500 text-3xl mb-4"></i><br>เกิดข้อผิดพลาดในการโหลดข้อมูลจาก Excel';
            }
        }

        // เริ่มดึงข้อมูลเมื่อเปิดเว็บ
        loadDataFromExcel();
        ...
        function renderHome() {
            let html = `
                <div class="mb-8">
                    <h2 class="text-3xl font-bold text-gray-900 mb-2">อัปเดตตอนใหม่ล่าสุด</h2>
                    <p class="text-gray-500">นิยายสนุกๆ มากมายรอคุณอยู่</p>
                </div>
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 sm:gap-6">
            `;

            if (novels.length === 0) html += `<div class="col-span-full py-20 text-center text-gray-500">ไม่มีนิยายในระบบ</div>`;
        ...
        function renderNovelDetail() {
            ...
            let chaptersHtml = novelChapters.length === 0 ? `<div class="p-8 text-center text-gray-500 bg-gray-50 rounded-lg">ยังไม่มีตอนให้เปิดอ่าน</div>` : '';
                ...
                chaptersHtml += `
                    <div onclick="readChapter('${ch.id}')" class="flex items-center justify-between p-4 bg-white border border-gray-100 hover:border-green-300 hover:shadow-md rounded-xl cursor-pointer transition-all mb-3 group">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <h4 class="font-bold text-gray-800 group-hover:text-green-600 transition-colors">ตอนที่ ${ch.chapterNumber}: ${ch.title}</h4>
                            </div>
                        </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-8">
                    <div class="flex flex-col md:flex-row">
                        ...
                        <div class="p-6 md:p-8 flex-1 flex flex-col">
                            ...
                            <p class="text-lg text-gray-600 mb-6"><i class="fa-solid fa-pen-nib mr-2"></i>ผู้แต่ง: ${novel.author}</p>
                            <div class="flex gap-6 mb-6 pb-6 border-b border-gray-100">
                                <div class="text-center">
                                    ...
                                    <div class="text-xs text-gray-500 uppercase tracking-wider">ยอดเข้าชม</div>
                                </div>
                            </div>
                            <h3 class="text-xl font-bold text-gray-900 mb-3">เรื่องย่อ</h3>
                            <p class="text-gray-600 leading-relaxed novel-text bg-gray-50 p-4 rounded-xl border border-gray-100">${novel.synopsis}</p>
                        </div>
                    </div>
                </div>
                <div class="max-w-4xl">
                    <h3 class="text-2xl font-bold text-gray-900 mb-6 flex items-center gap-2"><i class="fa-solid fa-list-ul text-green-600"></i> รายการตอนทั้งหมด</h3>
                ...
                <div class="${state.readerSettings.theme} min-h-screen -mt-4 sm:-mt-6 lg:-mt-8 -mx-4 sm:-mx-6 lg:-mx-8 p-4 sm:p-8 transition-colors duration-300" id="reader-container">
                    <div class="max-w-3xl mx-auto bg-black/5 backdrop-blur-sm rounded-2xl p-4 mb-8 flex flex-wrap justify-between items-center gap-4 sticky top-20 z-40">
                        <button onclick="viewNovel('${novel.id}')" class="flex items-center gap-2 font-medium hover:opacity-70">
                            <i class="fa-solid fa-arrow-left"></i> สารบัญ
                        </button>
                        <div class="flex items-center gap-4 bg-white/50 px-4 py-2 rounded-full">
                            <span class="text-sm font-bold opacity-70">ขนาดอักษร</span>
                        ...
                        <div class="flex items-center gap-2">
                            <button onclick="changeTheme('theme-light')" class="w-8 h-8 rounded-full border-2 border-gray-300 bg-white" title="สว่าง"></button>
                            <button onclick="changeTheme('theme-sepia')" class="w-8 h-8 rounded-full border-2 border-gray-300 bg-[#f4ecd8]" title="ถนอมสายตา"></button>
                            <button onclick="changeTheme('theme-dark')" class="w-8 h-8 rounded-full border-2 border-gray-500 bg-[#1a202c]" title="มืด"></button>
                        </div>
                    </div>
                    <div class="max-w-3xl mx-auto bg-transparent">
                        <h4 class="text-center font-bold opacity-60 mb-2">${novel.title}</h4>
                        <h1 class="text-3xl font-bold text-center mb-10 pb-6 border-b border-current border-opacity-20">ตอนที่ ${chapter.chapterNumber} : ${chapter.title}</h1>
                        ...
                        <div class="mt-16 pt-8 border-t border-current border-opacity-20 flex justify-between items-center pb-20">
                            ${prevCh ? `<button onclick="readChapter('${prevCh.id}')" class="px-6 py-3 rounded-full bg-current bg-opacity-10 font-bold"><i class="fa-solid fa-chevron-left"></i> กลับ</button>` : `<div></div>`}
                            ${nextCh ? `<button onclick="readChapter('${nextCh.id}')" class="px-6 py-3 rounded-full bg-green-600 text-white font-bold">ต่อไป <i class="fa-solid fa-chevron-right"></i></button>` : `<div class="opacity-50 font-bold">จบ</div>`}
                        </div>
                    </div>
                </div>
            `;
        ...
        window.viewNovel = function(id) {
            // ระบบบันทึกยอดวิวจริงไปที่ Google Sheets
            if (state.view === 'home') {
                // บวกตัวเลขในหน้าเว็บทันที
                let n = novels.find(n => n.id === id);
                if (n) n.views += 1;
                
                // ส่งคำสั่งไปอัปเดตยอดใน Excel (ทำงานอยู่เบื้องหลัง)
